<script lang="ts">
  ...
  import { SECTION_BY_ID } from '$lib/settings/sections'
  ...
  import Railways from '$lib/components/Railways.svelte'

  const CUSTOM_MODEL = '__custom__'

  // The dedicated settings page to render, from the route param (issue #344). The
  // grid at /settings links here; `meta` is this section's registry entry (title +
  // description shown in the header), or undefined for an unknown section (404).
  const section = $derived($page.params.section ?? '')
  const meta = $derived(SECTION_BY_ID[section])
  ...
  async function load() {
    ...
    // Only fetch the extra data the current page needs, rather than everything on
  ...
  // General page: org identity, model, review policy, and branch naming.
  async function save() {
    if (!settings) {
      return
    }
  ...
  // Workspace page: the agent's instructions, environment setup script, and config
  // repo (issue #344 groups everything workspace-related here). Env vars and network
  // access have their own save buttons (`saveEnv` / `saveNetwork`).
  let workspaceSavedAt = $state<string | null>(null)
  async function saveWorkspace() {
    ...
    workspaceSavedAt = new Date().toLocaleTimeString()
  }
  ...
  // Lazy-load the Tailscale status the first time the operator opens the Apps page
  // (it execs into the container, so we don't pay for it on every settings visit).
  $effect(() => {
    if (section === 'apps' && !tailscale && !tailscaleLoading) {
      loadTailscale()
    }
  })
  ...
  // Reload whenever the section changes, not just on mount: SvelteKit reuses this
  // component across `/settings/[section]` param changes, so a plain onMount would
  // never refetch a section's data when navigating straight from one page to another.
  $effect(() => {
    section
    load()
    if (section === 'updates') {
...
<!-- One dedicated settings page (issue #344), reached from the /settings grid. The
     header (back link + title + description) comes from the section registry; the
     content below renders the section matching the route param. -->
<div class="mx-auto max-w-4xl space-y-6 px-6 py-6">
  <div class="space-y-1">
    ...
    {:else}
      ...
      <p class="text-sm text-muted-foreground">Unknown settings page.</p>
    {/if}
  </div>
  ...
  <div class="space-y-6">
    ...
    <Card.Root>
      ...
      <Card.Content class="space-y-5">
        <div class="rounded-md border border-border p-3 text-sm text-muted-foreground">
          Claude credentials live on the
          <a href="/settings/llms" class="font-medium text-primary underline">LLMs</a>
          page, where you can add and prioritize several subscriptions, tokens, or API keys.
        </div>
        <div class="space-y-1.5">
    ...
    <Card.Root>
      <Card.Header>
        <Card.Title>Instructions, setup &amp; config repo</Card.Title>
        <Card.Description>
          What the agent knows and how its workspace is prepared. Save, then Recreate the workspace
          (below) to apply the setup script and config repo.
        </Card.Description>
      </Card.Header>
      <Card.Content class="space-y-5">
        <div class="space-y-1.5">
        ...
        <div class="space-y-1.5">
          <Label for="configrepo">Config repo URL (~/.claude)</Label>
        ...
        <div class="flex items-center gap-3">
          ...
          {#if workspaceSavedAt}<span class="text-sm text-muted-foreground">Saved at {workspaceSavedAt}</span>{/if}
        </div>
      </Card.Content>
    </Card.Root>
    {/if}
