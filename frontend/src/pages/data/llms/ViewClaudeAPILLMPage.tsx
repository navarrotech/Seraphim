...
export function ViewClaudeAPILLMPage(props: ViewProps) {
  ...
  const form = useForm<LlmWithRateLimits>({
    ...
    defaultValues: {
      ...
      type: existingLLM?.type ?? 'CLAUDE_API_KEY',
      name: existingLLM?.name ?? '',
      isDefault: existingLLM?.isDefault ?? props.isFirst ?? false,
      preferredModel: existingLLM?.preferredModel ?? SUPPORTED_MODELS_BY_LLM.CLAUDE_API_KEY?.[0] ?? '',
      apiKey: '',
    ...
    <div className='relaxed'>
      ...
      <div className='compact'>
        <Autocomplete
          ...
          >{ SUPPORTED_MODELS_BY_LLM.CLAUDE_API_KEY.map((model) => (
          ...
          <Information
            ...
            content={() => <div className='text-sm'>
              <ExternalLink href='https://console.anthropic.com/settings/keys'>
                Get your Claude API key here
              </ExternalLink>
              <div className='compact'>
                <p className='block mb-2'>Requirements for Claude API keys:</p>
                <ul className='list-disc list-inside mb-2'>
                  <li>Allow Messages API access</li>
                  <li>Allow model listing access</li>
                </ul>
              </div>
              </div>
            }
